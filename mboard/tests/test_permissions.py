...
class PermissionsAndSecurityTests(TestCase):
    ...
    def test_unauthenticated_user_cannot_access_protected_views(self):
        self.client = Client()
        protected_urls = [
            reverse("mboard:post_edit", args=[self.post.id]),
            reverse("mboard:post_delete", args=[self.post.id]),
            reverse("mboard:post_submit"),
        ]
    ...
    def test_post_templates_contains_token(self):
        ...
        for template in post_templates:
            with open(f"mboard/templates/mboard/{template}") as f:
...
class RateLimiterTests(TestCase):
    ...
    def test_repeated_post(self):
        for i in range(AUTHENTICATED_LIMIT + 1):
            response = self.client.post(reverse("mboard:post_submit"), data=self.post_data)
    ...
    def test_repeated_upvote(self):
        ...
        for i in range(AUTHENTICATED_LIMIT + 1):
            response = self.client.post(reverse("mboard:post_upvote", args=(post.id,)))
            if i < AUTHENTICATED_LIMIT:
